package highpeaktestcase;

import java.util.*;

public class Nesara1 
{
	...
	static int findMaxearningRec(Job arr[], int n) 
	{
		...
		int inclearning = arr[n - 1].earning;
		int i = Nonconflict(arr, n);
		if (i != -1)
			inclearning += findMaxearningRec(arr, i + 1);

		int exclearning = findMaxearningRec(arr, n - 1);

		return Math.max(inclearning, exclearning);
	}
